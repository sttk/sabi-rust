...
impl TxnDataHub {
    ...
    ///
    /// This method is similar to the global [`uses!`][crate::tokio::uses!] macro but registers a
    /// data source that is local to this specific [`DataHub`] session. Once the [`DataHub`]'s
    /// state in this [`TxnDataHub`] instance is "fixed" (while
    /// [`run_async`][TxnDataHub::run_async], [`start_async`][TxnDataHub::start_async],
    /// [`txn_async`][TxnDataHub::txn_async] or [`begin_txn_async`][TxnDataHub::begin_txn_async]
    /// method is executing),
...
impl<'a> Txn<'a> {
    ...
    /// [`run_async`][Txn::run_async] and [`run_or_block_async`][Txn::run_or_block_async] are
    /// skipped.
    ///
    /// # Parameters
    ///
    ...
    ///
    /// However, previous [`run_or_block_async`][Runner::run_or_block_async] failed, this execution
    /// is skipped.
    ///
    ...
    ///
    /// If the logic function fails, its error is collected and subsequent calls to
    /// [`run_async`][Txn::run_async], [`run_force_async`][Txn::run_force_async], and
    /// [`run_or_block_async`][Txn::run_or_block_async] are skipped.
    ///
    /// # Parameters
    ///
    /// * `logic_fn`: A closure that encapsulates the business logic to be executed.
