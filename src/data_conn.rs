...
impl DataConnList {
    ...
    #[inline]
    pub(crate) fn last(&self) -> *mut DataConnContainer {
        self.last
    }
...
mod tests_of_data_conn {
    ...
    impl<'a> SampleDataConn {
        fn new(id: i8, logger: Rc<RefCell<Logger>>) -> Self {
            Self { id, logger }
        }
    }
    ...
    impl Drop for SampleDataConn {
        fn drop(&mut self) {
            self.logger
                .borrow_mut()
                .log(&format!("SampleDataConn {} dropped", self.id));
        }
    }
