...
/// The enum type representing the reasons for errors that can occur within [`DataHub`] operations.
#[derive(Debug)]
...
impl DataHub {
    ...
    /// "fixed" (while [`run`][DataHub::run] or [`start`][DataHub::start] method is executing),
    ...
    /// registered or removed via [`disuses`][DataHub::disuses] method in subsequent operations.
    ///
    /// # Parameters
    ///
...
impl<'a> Runner<'a> {
    ...
    ///
    /// If the logic function fails, its error is collected and subsequent calls to
    /// [`run`][Runner::run], [`run_force`][Runner::run_force], and
    /// [`run_or_block`][Runner::run_or_block] are skipped.
    ///
    /// # Parameters
    ///
    /// * `logic_fn`: A closure that encapsulates the business logic to be executed.
