...
/// Is the enum type that represents the possible reasons for errors that can occur during
/// asynchronous operations managed by an `AsyncGroup`.
#[derive(Debug)]
pub enum AsyncGroupError {
    /// Indicates that it is failed to create a new runtime instance for an asynchronous operation.
...
impl<'a> AsyncGroup<'_> {
    ...
    ///
    /// # Parameters
    /// - `future_fn`: An async function that will be executed concurrently. It must
    ///   return a `Result<(), Err>`, where `Err` indicates a failure.
...
mod tests_of_async_group {
    ...
    fn multiple_processes_and_single_fail() {
        ...
        assert_eq!(
            ...
            "errs::Err { reason = sabi::async_group::tests_of_async_group::Reasons BadString(\"-\"), file = src/async_group.rs, line = ".to_string() + &(BASE_LINE + 68).to_string() + " }",
        );
        #[cfg(windows)]
        assert_eq!(
            format!("{:?}", *(m.get("bar").unwrap())),
    ...
    fn multiple_fail() {
        ...
        assert_eq!(
            ...
            "errs::Err { reason = sabi::async_group::tests_of_async_group::Reasons BadNumber(123), file = src/async_group.rs, line = ".to_string() + &(BASE_LINE + 101).to_string() + " }",
        );
        #[cfg(windows)]
        assert_eq!(
            format!("{:?}", *(m.get("baz").unwrap())),
